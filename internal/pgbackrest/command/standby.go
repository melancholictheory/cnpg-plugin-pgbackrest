...
type StandbyBackupTopology struct {
	...
	// Mode is the value passed to pgBackRest's --backup-standby option
	// ("y" or "prefer").
	Mode string
...
// should be configured to coordinate with the primary. This is true only when
// the feature is enabled (mode "y" or "prefer") and this instance is a standby
// with a known, different primary.
func ShouldConfigurePrimaryPeer(mode, currentPrimary, instanceName string) bool {
...
// BackupStandbyOption returns the --backup-standby flag for the given mode.
func BackupStandbyOption(mode string) string {
	return fmt.Sprintf("--backup-standby=%s", mode)
}
