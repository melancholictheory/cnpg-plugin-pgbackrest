...
var _ = Describe("Backup from standby options", func() {
	...
	Describe("BackupStandbyOption", func() {
		It("formats the --backup-standby flag", func() {
			Expect(BackupStandbyOption("y")).To(Equal("--backup-standby=y"))
			Expect(BackupStandbyOption("prefer")).To(Equal("--backup-standby=prefer"))
		})
	})
	...
	DescribeTable("ShouldConfigurePrimaryPeer",
		func(mode, currentPrimary, instanceName string, expected bool) {
			Expect(ShouldConfigurePrimaryPeer(mode, currentPrimary, instanceName)).To(Equal(expected))
		},
		Entry("disabled by empty mode", "", "cluster-1", "cluster-2", false),
		Entry("disabled explicitly", string(pgbackrestApi.BackupStandbyDisabled), "cluster-1", "cluster-2", false),
		Entry("enabled but no primary known yet", "y", "", "cluster-2", false),
		Entry("enabled but this instance is the primary", "y", "cluster-1", "cluster-1", false),
		Entry("enabled and this instance is a standby", "y", "cluster-1", "cluster-2", true),
		Entry("prefer and this instance is a standby", string(pgbackrestApi.BackupStandbyPrefer), "cluster-1", "cluster-2", true),
	)
})
