...
// StandbyBackupConfiguration configures taking backups from a standby instance
// instead of the primary. When set and a backup is scheduled on a replica, the
// plugin points pgBackRest at the current primary over TLS and passes
// --backup-standby, offloading the backup I/O from the primary.
type StandbyBackupConfiguration struct {
	// Mode mirrors pgBackRest's backup-standby option: "y" requires the backup
	// to run on a standby, "prefer" uses a standby when one is available and
	// otherwise falls back to the primary.
	// +kubebuilder:validation:Enum=y;prefer
	Mode BackupStandbyType `json:"mode"`

	// ManageService controls whether the plugin injects the headless service
	// used to reach the current primary's pgBackRest server. Defaults to true;
	// set it to false to provision and manage that service yourself.
	// +optional
	ManageService *bool `json:"manageService,omitempty"`

	// ManageCertificate controls whether the plugin injects the certificate
	// alternative name for that service into the cluster (serverAltDNSNames).
	// Defaults to true; set it to false to manage that entry yourself.
	// +optional
...
type PgbackrestConfiguration struct {
	...
	// BackupStandby, when set, makes backups that are scheduled on a standby run
	// in pgBackRest's "backup from standby" mode, offloading the backup I/O from
	// the primary. Leave it unset to keep the primary-only behavior.
	//
	// Experimental: this additionally requires a pgBackRest TLS server and its
	// certificates on the instances. See
	// https://github.com/operasoftware/cnpg-plugin-pgbackrest/issues/103
	// +optional
	BackupStandby *StandbyBackupConfiguration `json:"backupStandby,omitempty"`
}
